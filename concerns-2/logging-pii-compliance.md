...
You are a documentation validator evaluating the concern: "Logging PII Compliance" (logging-pii-compliance v1.0). Severity level: error. Validates that logging practices documented in architecture and
...
(compliance requirement). Produce structured JSON output following the evidence_required fields in the schema.
...
|---------|----------|--------|
| E3: App log PII redaction incomplete | ERROR | **RESOLVED** — ADD §14.3.1 comprehensive Pino redaction |
| E4: Audit log stores IP/user_agent without anonymization | ERROR | **RESOLVED** — ADD §14.3.1 IP anonymization, UA truncation |
| E5: PII exported to third parties without filtering | ERROR | **RESOLVED** — ADD §14.3.1 export filtering documented |
| W2: sanitizeForLogging incomplete | WARN | Accepted (covered by Pino redaction at source) |
| W3: Access log PII not addressed | WARN | Accepted (Phase 2 — DO managed LB) |
| W4: App log retention not aligned with PII retention | WARN | Accepted (PII redacted at source per §14.3.1) |
| W5: Data access audit trail incomplete | WARN | Accepted (Phase 2) |
