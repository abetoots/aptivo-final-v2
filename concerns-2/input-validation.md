...
You are a documentation validator evaluating the concern: "Input Validation at System Boundaries" (input-validation v1.0). Severity level: warn. Every system boundary where external (untrusted) data enters must have
...
verify what they haven't specified. Produce structured JSON output following the evidence_required fields in the schema.
...
|---------|----------|--------|
| W11: Inbound webhook payload size limit | WARN | Accepted (ADD §14.7 residual risk) |
| W12: Global API body size limit | WARN | Accepted (framework defaults) |
| W13: LLM output validation | WARN | Accepted (ADD §14.5 residual risk) |
| W14: MCP tool response size limits | WARN | Accepted (ADD §14.4 residual risk) |
