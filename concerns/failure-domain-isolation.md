...
resolution:
  ...
  findings:
    ...
    notes: 0
  remediation:
    - target: "docs/03-architecture/platform-core-add.md"
      action: "Added §2.3 Failure Domain Map with 12 components fully documented"
      sections_added:
        - "§2.3.1 Component Criticality Classification"
        - "§2.3.2 Failure Domain Matrix (12 components)"
      principle_added: "§2.2 item 3 — Failure Domain Isolation"
    - target: "docs/06-operations/01-runbook.md"
      action: "Added incident playbooks and recovery priority"
      sections_added:
        - "§8.7 Playbook 4: Redis Outage"
        - "§8.8 Playbook 5: External SaaS Outage"
        - "§8.9 Component Criticality & Recovery Priority"
  accepted_risks:
    - "PostgreSQL single instance (Phase 1) — cascading failure domain"
    - "Redis single node (Phase 1) — shared infrastructure"
    - "Single-region deployment (Phase 1) — no multi-region DR"
  phase2_upgrades:
    - "HA-tier PostgreSQL with standby failover"
    - "Separate Redis instances for queues vs cache"
    - "Multi-region DR with DNS failover"
    - "Self-hosted Novu for notification independence"
  multi_review: "concerns/FAILURE_DOMAIN_ISOLATION_MULTI_REVIEW.md"
```
