...
resolution:
  ...
  remediation:
    - target: "docs/03-architecture/platform-core-add.md"
      action: "Updated §9.1 schema diagram with complete table ownership annotations"
    - target: "docs/03-architecture/platform-core-add.md"
      action: "Added §3.5 Workflow Data Ownership (definitions, execution state bridge, feature flags)"
  accepted_risks:
    - "PostgreSQL shared database (Phase 1) — documented in §2.3.2"
    - "Audit sync writes — documented gap with recommendation"
    - "Feature flags are code-defined, not runtime service (Phase 1)"
  multi_review: "concerns/STATE_OWNERSHIP_CLARITY_MULTI_REVIEW.md"
```
