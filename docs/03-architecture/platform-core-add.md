...
| WS Multi-Instance Transport (TCP Redis Streams) | **TCP Redis (Railway-managed) via `ioredis`** for `WS_TRANSPORT_MODE=streams\|dual` | Upstash REST has no Streams + no persistent SUBSCRIBE — broken multi-instance fan-out. TCP Redis Streams + per-instance consumer groups (`ws-instance-<id>`) give every ws-server instance its own cursor over a single XADD-written stream → broadcast fan-out (AD-S18-2). Default `list` mode keeps Upstash-only deployments working. | Per-instance fan-out queues with Inngest (cardinality fixed; manual config edits to scale), HTTP-streamed Server-Sent Events (different protocol surface), Redis Pub/Sub on TCP Redis (lossy on subscriber crash) | `WsRedisClient` interface in `@aptivo/redis` is narrow (xadd/xreadgroup/xgroup + SET-NX-EX); swap to a different streams backend by writing one impl. ioredis is `optionalDependency`; package ships even where TCP Redis isn't provisioned. | Claude (S18 plan), user via AskUserQuestion 2026-04-28 (D1 = "TCP Redis + Streams (Recommended)"), ratified by Gemini + Codex (S18 plan + B1/B2 multi-reviews). Staging deploy verification deferred to S19 (provisioning calendar dependency on DevOps; package + scaffolding ship in S18). |
...
>
> **Update (Phase 3, Sprint 17 — 2026-04-28)**: Item (1) is fully resolved; the silent-fallback gap is fully resolved; items (2)-(3) shipped a **contract layer** but production paths carry to S18; (4) Replicate remains external. Specifically: (1) `FeatureFlagService.peekEnabled` ships in S17-B2 (`03b19d0`); safety gates rebound in `services.ts`. (2) Anomaly-gate aggregate-key alignment ships in S17-B1 (`ecb4792`) as contract layer (per-domain action whitelist + scope-key formatter); the `WHERE user_id = $actor` filter matches zero rows today because Inngest workflow callsites emit `actor.type='system'` — production closure requires workflow→user actor propagation in S18. (3) `CompletionRequest.actor` + `requireLlmContext` middleware ship in S17-B1 (`ecb4792`) but no production HTTP path consumes the middleware yet (workflow callsites don't stamp `request.actor`); production closure carries to S18. Silent-fallback gap closed by S17-B4 (`8bbaa43`): `ml_classifier_timeout` SLO evaluator (5%/5-min, 20-sample noise filter) + runbook §5.2.1. Real anomaly baseline job (S17-B3 `ea986f4`) replaces the placeholder constant; baseline values are operationally meaningful only after Gate #2/#3 production flow lands. (4) Replicate procurement remains an external finance/procurement track. **Production flag-flip readiness: `ml-injection-classifier` blocked on Replicate; `anomaly-blocking` blocked on S18 actor-propagation observation.** Full audit in [sprint-17-delivery-review.md §6](../06-sprints/sprint-17-delivery-review.md#6-enablement-gates--final-status-closes-s16-6).
