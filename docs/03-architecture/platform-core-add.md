...
>
> **Update (Phase 3, Sprint 17 — 2026-04-28)**: Items (1)-(3) plus the silent-fallback alert gap are **resolved**. (1) `FeatureFlagService.peekEnabled` ships in S17-B2 (`03b19d0`); safety gates rebound in `services.ts`. (2) Anomaly-gate aggregate-key alignment ships in S17-B1 (`ecb4792`) via per-domain action whitelist. (3) `CompletionRequest.actor` + `GatewayDeps.resolveActor` ship in S17-B1 (`ecb4792`); `llm_usage_logs.departmentId` populated end-to-end. Silent-fallback gap closed by S17-B4 (`8bbaa43`): `ml_classifier_timeout` SLO evaluator (5%/5-min, 20-sample noise filter) + runbook §5.2.1. Real anomaly baseline job (S17-B3 `ea986f4`) replaces the placeholder constant. (4) Replicate procurement remains an external finance/procurement track. Engineering otherwise unblocked for `ml-injection-classifier` + `anomaly-blocking` flag flips. Full closure audit in [sprint-17-delivery-review.md §6](../06-sprints/sprint-17-delivery-review.md#6-enablement-gates--final-status-closes-s16-6).
