...
**Purpose:** Centralized management of candidate data, workflows, interviews, and contracts.
**Success Metric (from BRD):** Reduce time-to-hire by 25% within 12 months of launch.

#### CM1: Centralized Candidate Repository

**Requirement:** The system shall provide a centralized repository for all candidate data.

**Acceptance Criteria:**
- The system stores candidate profiles including: name, contact details, resume/CV, application status, skills, and interview history.
- Each candidate record has a unique identifier.
- Users can search candidates by name, skills, status, or date range.
- Duplicate candidate detection alerts users when a candidate with matching email or phone already exists.
- All candidate data modifications are logged with timestamp and user.

#### CM2: Workflow & Status Management

**Requirement:** The system shall support customizable candidate workflows with visual stage management.

**Acceptance Criteria:**
- A default candidate lifecycle is provided: New → Under Review → Interview Scheduled → Offer Extended → Hired.
...
#### CM3: Interview Process Management
...
#### CM4: Contract Drafting & Compliance
...
- Contract templates support variable fields (candidate name, salary, start date, benefits, equity).
...
- Contracts include compliance flags for Philippine labor law requirements (mandatory benefits, probation terms).
- The system prevents sending contracts without required compliance fields completed.

#### CM5: Data Privacy & Consent (NEW)
...
- The system supports "Right to be Forgotten" requests: authorized users can anonymize a candidate record upon verified request.
- Anonymization removes PII while retaining anonymized aggregate data for analytics.
- An export function generates a machine-readable file (JSON/CSV) of all candidate data for Subject Access Requests (SAR).
